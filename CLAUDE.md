...
- 许可证：AGPLv3
- 版本以 `metadata.yaml` 与 `main.py` 的 `@register(..., "x.y.z")` 为准（当前 v2.1.4）
...
聊天平台 ──► AstrBot ──► HapiConnectorPlugin (main.py)
          ...
          ┌───────────────────┼───────────────────┐
          ▼                   ▼                   ▼
   CommandHandlers      LLMIntegration      快捷前缀 / 戳一戳
   (/hapi 路由)         (FC 工具 + 审批)     (quick_prefix / poke)
          │                   │
          └─────────┬─────────┘
                    ▼
...
| 模块 | 职责 |
|------|------|
| `main.py` | 插件入口 `HapiConnectorPlugin(Star)`：组装依赖、生命周期、`/hapi` 命令、LLM 工具代理、戳一戳、快捷前缀；含 AstrBot File 组件 setattr 兼容补丁 |
...
| `sse_listener.py` | 后台 SSE：权限请求、消息推送级别、自动审批时段、重连/休眠、待审批序号池 |
...
| `formatters.py` | 用户可见文案、帮助、session 标题 `get_session_title`、审批/列表格式 |
| `flavor_profiles.py` | Agent flavor 能力表：权限/模型/effort/plan/可创建；未知类型降级 |
...
当前仓库可能检出 `webui` 等功能分支。`dev-docs/plugin-pages.md` 描述 AstrBot **插件 Pages**（`pages/<name>/index.html` + `context.register_web_api` + `window.AstrBotPluginPage` bridge）。实现 WebUI 时遵循该文档；配置项优先 `_conf_schema.json`，复杂交互再上 Pages。

## 文档地图

| 路径 | 内容 |
|------|------|
| `README.md` | 功能、配置项、完整指令、SSE 级别、插件结构 |
| `CHANGELOG.md` | 版本行为变更（resume、plan/effort、FC 工具、隔离模型） |
| `docs/install.md` | HAPI Hub 安装与连接 |
| `docs/session-isolation.md` | 多窗口隔离规则 |
| `docs/cf_access_guide.md` | Cloudflare Access 配置 |
| `dev-docs/plugin-pages.md` | AstrBot 插件页面 API（开发中参考） |
...
- **无自动化测试**：改审批、SSE 完成态、resume、绑定路由等路径时，按场景做手动联调（active/inactive、question vs 普通权限、多窗口）。
